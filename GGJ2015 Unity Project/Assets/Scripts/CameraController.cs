...
public class CameraController : MonoBehaviour {
    ...
    void Update()
    {
        if (player)
        {
            ...
            {
                transform.position = new Vector3(player.transform.position.x, player.transform.position.y, -20) + (Vector3)offset;
            }
        }
        else
        {
    ...
    public IEnumerator FadeOutCoroutine(float speed)
    {
        Color color = fadeViewBlocker.renderer.material.color;
        while (color.a < 1)
        {
